...
在区块链领域，代币标准扮演着至关重要的角色，它定义了代币如何被创建和交互。ERC1155 是一种在[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)区块链上普遍使用的代币标准，它带来了多功能性和灵活性，允许一个[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)同时管理多种类型的代币，不论是同质化代币（FTs）还是非同质化代币（NFTs）。

## 理解 ERC1155
背景：在ERC1155 出现之前，ERC20 和 ERC721 是最常用的两种代币标准，分别代表同质化和非同质化代币。[ERC20](https://learnblockchain.cn/tags/ERC20?map=EVM) 代币因完全可互替而广受欢迎，适合用作货币、积分系统等；而 [ERC721](https://learnblockchain.cn/tags/ERC721?map=EVM) 每个代币都是唯一的，非常适合代表所有权，如艺术品、收藏品等。虽然这两种标准功能强大，但它们无法在一个合约中同时支持多种代币类型，这就是 ERC1155 发挥作用的地方。
...
这个方法允许用户通过输入一个代币的 ID 来获得一个 URI。这个 URI 提供了指向元数据的链接，用户可以通过这个链接访问代币的详细信息。URI 可以是一个 HTTP/HTTPS 链接，也可以是一个 [IPFS](https://learnblockchain.cn/tags/IPFS) 链接，甚至是其他形式的 URI。
...
**安全说明**：
- `mint` 函数使用了 `onlyOwner` 修饰符，确保只有合约所有者才能铸造新代币
- 在生产环境中，应该根据实际需求设计更复杂的访问控制机制，例如使用 OpenZeppelin 的 `AccessControl` 合约实现基于角色的权限管理
