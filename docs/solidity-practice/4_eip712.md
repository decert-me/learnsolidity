...
在[数字签名](https://learnblockchain.cn/article/22613)一文中，我们学习了如何对消息进行签名和验证。但在实际应用中，我们经常需要对复杂的结构化数据进行签名，比如订单、投票、授权等。
...
在[数字签名](https://learnblockchain.cn/article/22613)中，我们使用的是简单消息签名：
...
1. `\x19\x01`:
    ...
    更多签名数据标准参考 [EIP-191](https://learnblockchain.cn/docs/eips/EIPS/eip-191/)
...
接下来，我们将给出一个示例[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)，展示如何在合约中集成 EIP-712 签名机制。这个合约将包括创建 EIP-712 域分隔符，结构化数据的哈希，以及验证签名的功能。
...
## 参考
- [数字签名](https://learnblockchain.cn/article/22613) - 基础签名机制
- [EIP-712 类型化结构化数据散列和签名](https://learnblockchain.cn/docs/eips/EIPS/eip-712/)
- [EIP-191 签名数据标准](https://learnblockchain.cn/docs/eips/EIPS/eip-191/)

## 总结

EIP-712 是[基础消息签名](https://learnblockchain.cn/article/22613)的进阶版本，它为结构化数据签名提供了标准化的解决方案。

**核心优势**：
- 📊 **可读性**：用户能在[钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)中看到具体的数据结构和值，而不是一串哈希
- 🔒 **安全性**：通过域分隔符（Domain Separator）防止跨链、跨合约的签名重放攻击
- 🎯 **标准化**：统一的签名格式，[钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)和 [DApp](https://learnblockchain.cn/tags/DApp) 都能正确解析和展示
