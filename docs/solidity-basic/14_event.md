...
事件是以太坊上一个比较特殊的机制，以太坊虚拟机是一个封闭的沙盒环境，我们在EVM内部通过调用外部世界的接口，把信息转递给外部或从外部获得信息，因为[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)没法对外部的信息达成共识。想象一下，你有一个[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)，向一个网络API发出API请求，以获得一对资产的最新价格。当节点A处理一个触发这个API调用的交易时，得到响应`42`，并相应地更新合约状态。然后当节点B处理同样的交易时，价格发生了变化，响应是`40`，并相应地合约状态。然后节点C发出请求时，收到一个`404`的HTTP响应。当网络中的每个节点都可能对最新状态有不同的看法时，以太坊世界计算机就无法对最新状态达成共识。
...
对于生产环境，通常使用 [Web3](https://learnblockchain.cn/tags/Web3?map=Roadmap).js、Ethers.js 等库来获取和监听事件。具体使用方法可以参考：
...
有不少刚转入Web3 的工程师，把智能合约当成数据库使用，习惯把需要用到的数据都保存在[智能合约](https://learnblockchain.cn/tags/%E6%99%BA%E8%83%BD%E5%90%88%E7%BA%A6)中，但最佳的实践是：**如无必要，勿加存储。**

倘若在合约中，**没有任何函数读取该变量，我们应该使用事件来存储数据**，[Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 成本会降低很多。
...
使用事件的版本 [Gas](https://learnblockchain.cn/tags/Gas?map=EVM) 消耗远低于使用状态变量的版本。
