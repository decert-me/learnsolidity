...
当需要存储任意长度的字节数据时，优先使用 `bytes` 而不是 `bytes1[]`，因为 `bytes` 更加紧凑，能节省 Gas 费用。
...
`string` 类型**没有** `length` 属性，也**不支持**索引访问。如果需要操作字符串的字节，需要先转换为 `bytes`。
...
字符串比较不能直接使用 `==`，而是通过比较两者的 `keccak256` 哈希值来判断是否相等。
